...
def regresion_lineal_multiple(X_train, y_train, X_test):
    ...
    n = X_train.shape[0] # n = número de filas de X_train
    ones = np.ones((n, 1)) # Se creau una matriz (n, 1) de unos (columna de unos)
    X_train_b = np.hstack([ones, X_train]) # Se añade columna de unos a X_train para el intercepto

    # Paso 2 — Calcular los coeficientes β con la fórmula OLS
    # β = (XᵀX)⁻¹ Xᵀy
    XT = X_train_b.T # Matriz transpuesta (p+1, n)
    XTX = XT @ X_train_b # Multiplicación de matrices (devuelve matriz cuadrada -> Invertible) (p+1, p+1)
    XTX_inv = np.linalg.inv(XTX) # Matriz inversa

    XTy = XT @ y_train # Multiplicamos por el array de variables obteivo de entrenamiento (resutaldo como array 1D) (p+1,)
    coefs = XTX_inv @ XTy # Resultado array de coeficientes (p+1,)
    ...
    y_pred = X_test_b @ coefs # Array de predicciones 1D (n_test,)
...
def calcular_mae(y_real, y_pred):
    ...
    # La funcion se puede reconfigurar como (Σ |y_real - y_pred|) / n
    # Sumar los n valores y dividir por n es calcular la media
